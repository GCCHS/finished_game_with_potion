...
potionSprite = new Image();
potionSprite.src = "images/blue_potion.png";
potionSprite.addEventListener("load", init, false);
...
Player.prototype.draw = function () {
    this.drawAllBullets();
    ctxEntities.drawImage(imgSprite,
        this.srcX,
        this.srcY,
        this.width,
        this.height,
...
Player.prototype.checkDirection = function () {
    ...
    var touchingPotion = this.checkTouchingPotion(newDrawX, newDrawY);
    console.log("Touching", touchingPotion)
    if(touchingPotion){
        //potion.pickedUp = true;
        potion.checkHadEffect(this);
    }
...
Player.prototype.checkTouchingPotion = function (newDrawX, newDrawY) {
    var newCenterX = newDrawX + (this.width / 2);
    var newCenterY = newDrawY + (this.height / 2);
    var potionX = potion.drawX;
    var potionY = potion.drawY;
    var potionWidth = potion.width;
    var potionHeigh = potion.height;
    if (potionX < newCenterX
        && newCenterX < (potionX + potionWidth)
        && potionY - 20 < newCenterY
        && newCenterY < (potionY + potionHeigh) - 20)
    {
        return true
    }else{
        return false
...
Potion.prototype.draw = function(){
    if(!this.pickedUp){
        ctxEntities.drawImage(potionSprite, this.drawX, this.drawY);
    }
};

Potion.prototype.checkHadEffect = function(player){
    if(player.dHeight <= 108){
        player.dWidth += 2;
        player.dHeight += 2;
    }else{
        this.drawX = -1;
